...
import com.itdeep.micorservices.gamification.domain.Badge;
...
import com.itdeep.micorservices.gamification.domain.ScoreCard;
import com.itdeep.micorservices.gamification.repository.BadgeCardRepository;
...
/**
 * Calculate score and badges, based on the received attempts.
 *
 * @author od
 */
@Service
public class GameServiceImpl implements GameService {
    ...
    public GameStats newAttemptForUser(final Long userId,
        ...
        if (correct) {
            ...
            LOGGER.info("User with id {} scored {} points for attempt id {}",
    ...
    private List<BadgeCard> processForBadges(final Long userId, final Long attemptId) {
        ...
        List<BadgeCard> badgeCards = new ArrayList<>(Arrays.asList(Badge.BRONZE_MULTIPLICATOR, Badge.SILVER_MULTIPLICATOR, Badge.GOLD_MULTIPLICATOR)
                .stream()
                .map(badge -> checkAndGiveBadgeBasedOnScore(badgeCardList, badge,
                totalScore, badge.getScore(), userId))
                .filter(Optional::isPresent)
                .map(Optional::get)// the filter and map can be replaced by .flatMap(o -> o.map(Stream::of).orElseGet(Stream::empty))
                .collect(Collectors.toList()));
     ...
     * thresholds to gain badges. It also assigns badge to user if the
     * conditions are met.
     */
